...
// Strip quotes defensif — sama seperti di client.ts
...
if (typeof window !== "undefined") {
  console.info(
    `[Campfire/catalog] mode = ${USE_MOCK ? "MOCK" : "SUPABASE"} | NEXT_PUBLIC_USE_MOCK = "${process.env.NEXT_PUBLIC_USE_MOCK}"`
  )
}
...
export async function getProducts(): Promise<ProductCatalogItem[]> {
  if (USE_MOCK) return mockData.productCatalog
