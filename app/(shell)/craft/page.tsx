...
import type { UserProfileRow } from "@/lib/api/profile"
...
export default function CraftPage() {
  ...
  const [senderProfile, setSenderProfile] = useState<UserProfileRow | null>(null)
  ...
  useEffect(() => {
    ...
    // Load sender identity — non-blocking, fails gracefully if not logged in
    getUserProfile().then(p => { if (p) setSenderProfile(p) }).catch(() => {})
    ...
    <div className="p-8 max-w-4xl mx-auto space-y-10 animate-in fade-in duration-500">
      ...
      <div className="flex items-start justify-between border-b pb-6 border-border/40">
        ...
        <div className="flex items-center gap-2">
          <PageHelp
            title="Craft — Generate Email Campaign"
            content={{
              what: "AI menyusun 3 email campaign berdasarkan pain point, produk yang dipilih, dan konteks bisnis target.",
              tips: "Proses generate memakan waktu 30–60 detik. Jangan tutup halaman selama proses berlangsung.",
              next: "Setelah email selesai digenerate, lanjut ke Polish untuk mengedit dan finalisasi."
            }}
          />
      ...
      <div className="space-y-6 pt-2">
        ...
        <div className="space-y-6">
          {campaign.emails.map((email: any) => (
            <EmailPreviewCard
              key={email.id ?? email.sequenceNumber}
              email={email}
              senderName={senderProfile?.sender_name}
              senderTitle={senderProfile?.sender_title}
            />
          ))}
        </div>
      </div>
