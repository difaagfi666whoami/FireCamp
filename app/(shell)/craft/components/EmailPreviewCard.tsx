...
export function EmailPreviewCard({ email, senderName, senderTitle }: EmailPreviewCardProps) {
  const hasSenderInfo = !!(senderName?.trim() || senderTitle?.trim())
  ...
  return (
    <Card className="border-border/80 shadow-sm relative pt-4 overflow-hidden rounded-[14px]">
      ...
      <CardHeader className="py-4 px-6 border-b border-border/40 bg-slate-50/50">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground bg-white border border-border/60 px-3 py-1.5 rounded-full shadow-sm transition-colors"
            >
      ...
      <CardContent className="p-6">
        ...
        {hasSenderInfo && (
          <p className="mt-3 text-[12px] text-muted-foreground font-medium text-right">
            — {[senderName, senderTitle].filter(Boolean).join(", ")}
          </p>
        )}
      </CardContent>
    </Card>
